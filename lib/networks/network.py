...
class Network(object):
    ...
    def build_loss(self, ohem=False):
        # classification loss
        rpn_cls_score = tf.reshape(self.get_output('rpn_cls_score_reshape'), [-1, cfg.NCLASSES])  # shape (HxWxA, 2)
        rpn_label = tf.reshape(self.get_output('rpn-data')[0], [-1])  # shape (HxWxA)
        
        # ignore_label(-1)
        fg_keep1 = tf.equal(rpn_label, 1) # keep latin
        fg_keep2 = tf.equal(rpn_label, 2)  # keep asian
        ...
        rpn_loss_box = tf.reduce_sum(rpn_loss_box_n) / (tf.reduce_sum(tf.cast(fg_keep1, tf.float32))
                                                        + tf.reduce_sum(tf.cast(fg_keep2, tf.float32))+ 1)
        ...
        return total_loss,model_loss, rpn_cross_entropy, rpn_loss_box,\
               rpn_label, rpn_cls_score, rpn_bbox_pred, rpn_bbox_targets

...
def anchor_target_layer(rpn_cls_score, gt_boxes, gt_ishard, dontcare_areas, im_info, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    #至此，anchor准备好了
    #--------------------------------------------------------------
    # label: 1 is positive, 0 is negative, -1 is dont care //// update: 1 is lang1, 2 is lang2, 0 is negative, -1 is dont care
    # (A)
    ...
    for i,o in enumerate(gt_argmax_overlaps):
        if i <= len(gt_boxes) - 1:
            if gt_boxes[i][4] == 2:
                labels[o] = 2
            elif gt_boxes[i][4] == 1:
    ...
    for i,o in enumerate(list(max_overlaps)):
        if o >= cfg.TRAIN.RPN_POSITIVE_OVERLAP and gt_boxes[argmax_overlaps[i]][4] == 2:
            labels[i] == 2
        elif o >= cfg.TRAIN.RPN_POSITIVE_OVERLAP and gt_boxes[argmax_overlaps[i]][4] == 1:
            labels[i] == 1
    ...
    # let's make it 'perfectly balance'
    min_num = np.min([len(np.where(labels == 0)[0]), len(np.where(labels == 1)[0]), len(np.where(labels == 2)[0])])

    num_fg1 = min_num
    fg1_inds = np.where(labels == 1)[0]
    if len(fg1_inds) > num_fg1:
        disable_inds = npr.choice(
            fg1_inds, size=(len(fg1_inds) - num_fg1), replace=False)#随机去除掉一些正样本
        labels[disable_inds] = -1#变为-1

    num_fg2 = min_num
    fg2_inds = np.where(labels == 2)[0]
    if len(fg2_inds) > num_fg2:
        disable_inds = npr.choice(
            fg2_inds, size=(len(fg2_inds) - num_fg2), replace=False)  # 随机去除掉一些正样本
        labels[disable_inds] = -1  # 变为-1
    ...
    print('---- gt label----')
    print('negative: '+str(len(np.where( labels == 0)[0])))
    print('dontcare: '+str(len(np.where( labels == -1)[0])))
    print('latin: '+str(len(np.where( labels == 1)[0])))
    print('asian: '+str(len(np.where( labels == 2)[0])))

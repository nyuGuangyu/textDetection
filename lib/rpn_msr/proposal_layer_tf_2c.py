...
from lib.fast_rcnn.config_2c import cfg
...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    scores0 = np.reshape(np.reshape(rpn_cls_prob_reshape, [1, height, width, _num_anchors, n_classes])[:,:,:,:,0],
    ...
    if pre_nms_topN > 0:                #保留12000个proposal进去做nms
        ...
        # order_lang1 = list(order_lang1) 
        # order_lang2 = list(order_lang2)
        
        # # 去除重合
        # cnt = 0
        # temp1 = []
        # temp2 = []
        # # print(set(order_lang2)==set(order_lang1))
        # # sys.exit()
        # for i in range(len(order_lang1)):
        #     if order_lang1[i] >= order_lang2[i]:
        #         temp1.append(i)
        #     else:
        #         temp2.append(i)
        #     cnt += 1
        #     if cnt >= pre_nms_topN:
        #         break
        # order = list(set(temp1) | set(temp2))
        # order_lang1 = np.array(temp1)
        # order_lang2 = np.array(temp2)

 
    proposals = proposals[order, :]
    scores = scores[order]
    # a = np.zeros((len(order),1))
    # a[order_lang1] = scores_lang1[order_lang1]
    # scores_lang1 = a

    # b = np.zeros((len(order),1))
    # b[order_lang2] = scores_lang2[order_lang2]
    # scores_lang2 = b
    ...
    if post_nms_topN > 0:
        ...
        # keep_lang1 = list(keep_lang1) 
        # keep_lang2 = list(keep_lang2)
        # # 去除重合
        # cnt = 0
        # temp1 = []
        # temp2 = []
        # # print(set(order_lang2)==set(order_lang1))
        # # sys.exit()
        # for i in range(len(keep_lang1)):
        #     if keep_lang1[i] >= keep_lang2[i]:
        #         temp1.append(i)
        #     else:
        #         temp2.append(i)
        #     cnt += 1
        #     if cnt >= post_nms_topN:
        #         break
        # keep = list(set(temp1) | set(temp2))
        # keep_lang1 = np.array(temp1)
        # keep_lang2 = np.array(temp2)
    ...
    # a[keep_lang1] = scores_lang1[keep_lang1]
    # scores_lang1 = a

    # b = np.zeros((len(keep),1))
    # b[keep_lang2] = scores_lang2[keep_lang2]
    # scores_lang2 = b

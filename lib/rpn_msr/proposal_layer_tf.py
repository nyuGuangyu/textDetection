...
def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride = [16,], anchor_scales = [16,]):
    ...
    order_lang1 = scores_lang1.ravel().argsort()[::-1] 
    ...
    if pre_nms_topN > 0:                #保留12000个proposal进去做nms
        ...
        # 去除重合
        order = order[:pre_nms_topN]
        order_lang1 = order_lang1[:pre_nms_topN]
        order_lang2 = order_lang2[:pre_nms_topN]  
 
    proposals = proposals[order, :]
    #scores = scores[order]
    a = np.zeros((len(order),1))
    if order_lang1:
        a[:int(.5*len(order))] = scores_lang1[order_lang1][:int(.5*len(order))]
    scores_lang1 = a

    b = np.zeros((len(order),1))
    if order_lang2:
        b[:int(.5*len(order))] = scores_lang2[order_lang2][:int(.5*len(order))]
    scores_lang2 = b
